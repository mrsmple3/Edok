				...
				<button class="submenu-parent relative flex-center gap-[11px] rounded-[14px] border border-[#2d9cdb] py-2 px-7 text-[#2d9cdb] text-[18px] font-bold font-['Barlow'] mr-[24px] hover:active">
					...
					<div class="submenu">
						<div class="cursor-pointer">
							...
							<input id="specification" type="file" accept="application/pdf" class="hidden" @change="(event) => handleFileUpload(event, 'Спецификация')"/>
						</div>
						<div class="cursor-pointer">
							<label for="check">Добавить счет</label>
							<input id="check" type="file" accept="application/pdf" class="hidden" @change="(event) => handleFileUpload(event, 'Счет')"/>
						</div>
						<div class="cursor-pointer">
							<label for="invoice">Добавить накладную</label>
							<input id="invoice" type="file" accept="application/pdf" class="hidden" @change="(event) => handleFileUpload(event, 'Накладная')"/>
						</div>
						<div class="cursor-pointer">
							<label for="confirming">Добавить подтверждающие документы</label>
							<input id="confirming" type="file" accept="application/pdf" class="hidden" @change="(event) => handleFileUpload(event, 'Подтверждающий документ')"/>
						</div>
					</div>
				</button>
